<template>
  <div>
    <Title>标签"{{ route.params.name }}"下的文章 | {{ config.TITLE }}</Title>
...
// 获取指定标签的所有文章总数
const { data: totalCount } = await useAsyncData(`tag-${route.params.name}-count`, async () => {
  const count = await queryCollection('content')
    .where('tags', 'LIKE', `%${route.params.name}%`)
    .count();
  return count;
});
...
// 获取指定标签的所有文章
const { data: posts } = await useAsyncData<PostItem[]>(`tag-${route.params.name}`, async () => {
...
const handlePageChange = (page: number) => {
  router.push(`/tag/${route.params.name}/${page}`);
};
...
if (!posts.value?.length) {
  throw createError({
    statusCode: 404,
    message: `No posts found with tag "${route.params.name}"`
  });
}
</script>
